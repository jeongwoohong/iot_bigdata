...
//	int num2;
	...
	//4_precaution_for_division.c(16): warning C4477: 'printf' : 서식 문자열 '%f'에
    // 'double' 형식의 인수가 필요하지만 variadic 인수 3의 형식이 'int'입니다.
	//printf("%d/%d=%f\n",numerator, denominator, numerator/denominator);

	// 정수 연산 결과를 float로 타입캐스팅 시도
	// 컴파일러에 따라 warning, compile error 또는 runtime error를 피할 수
	// 있으나 정수형 결과를 단순히 변환했으므로 손실되는 갑이 존재한다. 
	...
	printf("%f/%f=%f\n",numerator,denominator, float numerator/ floatdenominator);
