...
void main() { // 함수 정의부
	// local varialbe 변수 선언부 (선언 또는 선언/초기화를 같이 한다.)
...
//	int cnt;
	...
	for (int cnt = 0; cnt < 3; cnt++) {
		...
		// ANSI-99 Style로 변수를 선언하는 것을 추천하지 않습니다.
		// Why? 유지보수성 저하 / 개발자 실수유발
		int num = 0;
		num++;
	...
	// 아래 코드는 컴파일 에러를 발생합니다.
//	printf("최종 num은 %d \n", num);
	// cnt도 for문 안의 {} scope에 해당한다.
//	printf("최종 cnt는 %d \n", cnt);

	// 이러한 {} Scope는 {}사용하는 모든 명령문에 동일하게 적용됩니다.
	// 아래 cnt는 main 함수의 최상위 {} scope를 가진다.
	// 반면 위에 cnt는 main 함수 밑에 for문 {}의 scope를 가진다.
    int cnt = 0;
	...
	// 접근한 결과는 유효하다.
	printf("최종 cnt : %d \n", cnt);
	// num은 while 내부의 scope이기 때문에 접근 불가능
	// printf("최종 num은 %d \n", num);
}